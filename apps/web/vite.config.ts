...
export default defineConfig(({ mode }) => {
  ...
  return {
    define: {
      ...
      "process.env.USER_OF_THE_WEEK_ID": JSON.stringify(env.VITE_USER_OF_THE_WEEK_ID || "-1"),
      "process.env.USER_OF_THE_WEEK_TEXT": JSON.stringify(env.VITE_USER_OF_THE_WEEK_TEXT || ""),
      "process.env.ADMIN_PANEL_URL": JSON.stringify(env.VITE_ADMIN_PANEL_URL || ""),
      "process.env.GUEST_REDIRECT_URL": JSON.stringify(env.VITE_GUEST_REDIRECT_URL || ""),
      "process.env.FINDRETROS_GUARD_ENABLED": JSON.stringify(env.VITE_FINDRETROS_GUARD_ENABLED || ""),
      "process.env.DISCORD_INVITE_URL": JSON.stringify(env.VITE_DISCORD_INVITE_URL || ""),
    },
    plugins: [solidPlugin()],
    server: {
      port: 4001,
